/*
 * C++ implementation of lapack routine dormqr
 *
 * $Id: dormqr.cpp,v 1.7 1993/07/07 23:00:32 alv Exp $
 *
 *************************************************************
 *
 ...
 *
 * Translated by FOR_C++, v1.1 (P), on 02/18/93 at 07:37:01
 ...
 *
 * Revision 1.6  1993/04/06  20:41:47  alv
 ...
 * made parms const
 *
 * Revision 1.2  1993/03/05  23:16:24  alv
 * changed ref parms to const ref
 *
 * Revision 1.1  1993/03/03  16:08:17  alv
 * Initial revision
 *
 */
...
RWLAPKDECL void /*FUNCTION*/ dormqr(const char &side, const char &trans, const long &m, const long &n, 
...
#define NCHRTMPS 1
    ...
    if( t == NULL )
        memerr( "unable to allocate object in dormqr()" );
    ...
    nb = min( NBMAX, ilaenv( 1, "DORMQR", f_concat(&_c[0],STR1(_c0,side),
    ...
    if( nb > 1 && nb < k ) { 
        ...
        if( lwork < iws ) { 
            nb = lwork/ldwork;
            nbmin = max( 2, ilaenv( 2, "DORMQR", f_concat(&_c[0],STR1(_c0,side),
                                                          STR1(_c1,trans),NULL), m, n, k, -1 ) );
        }
    }
    else { 
    ...
    if( nb < nbmin || nb >= k ) { 
        ...
        for( i = i1, i_ = i - 1, _do0=docnt(i,i2,_do1 = i3); _do0 > 0; i += _do1, i_ += _do1, _do0-- ) { 
            ...
            dlarft( 'F'/* Forward */, 'C'/* Columnwise */, nq - i + 
                    1, ib, &A(i_,i_), lda, &tau[i_], (double*)t, LDT );
            if( left ) { 
            ...
            dlarfb( side, trans, 'F'/* Forward */, 'C'/* Columnwise */
