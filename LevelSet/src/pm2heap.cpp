...
namespace levelset {
    ...
    void PolarMesh2D_Heap::Change(const int i, const double val, const int ikhi)
    {
        data[i] = val;
        SortFrom(i,ikhi);
    }
    
    void PolarMesh2D_Heap::Insert(const int i, const int j, const double val,
                                  const int ikhi)
    {
    ...
    void PolarMesh2D_Heap::Extract(int& i, int& j, const int ikhi)
    {
    ...
    void PolarMesh2D_Heap::SortFrom(unsigned int n, const int ikhi)
    {
        if (type == MinHeap) {
            int k;
    ...
    void PolarMesh2D_Heap::Dump(std::ostream& s, const char val) const
    {
        unsigned int depth = heapT_log2(length);
        if (depth < 1) {
            depth = 1;
            std::cout << "Oops Dump has depth 0\n";
        }
        int rows = int(0x01 << (depth-1));
        int width = 9;
        int prec = 3;
    ...
    void PolarMesh2D_Heap::Check(const int ikhi) const
    {
