...
namespace levelset {
    ...
    void UM2_MirrorBdry::Apply(const int k)
    {
        int i, j;
        
        for (i=-width[BDRY2D_XLO]; i<0; ++i)
            for (j=0; j<mesh->maxj; ++j) 
                mesh->mdata_(mesh,i,j,k) = mesh->mdata_(mesh,-i,j,k);
    ...
    void UM2_MirrorBdry::Apply(const int ic, const int jc, const int k)
    {
        std::cerr << "Error: MirrorBdry individual apply not yet implemented\n";
